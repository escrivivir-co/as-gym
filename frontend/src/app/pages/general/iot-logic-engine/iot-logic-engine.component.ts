...
import { RunStateEnum } from '@alephscript/mcp-core-sdk/browser';
...
import { DashboardComponent } from '../../application/feature/logic/dashboard/dashboard.component';

// Alias for backward compatibility
type IMundo = IMundoState;
...
@Component({
	...
	imports: [LoggerComponent, CommonModule, RouterLink, RouterOutlet,
		PrettyJsonPipeV2, DynamicFormComponent, BotoneraComponent,
		PantallaComponent,
		DashboardComponent],
	templateUrl: './iot-logic-engine.component.html',
	styleUrl: './iot-logic-engine.component.css',
...
export class IotLogicEngineComponent implements OnInit {
	...
	cargaChainState(d: IRuntimeBlock) {
		...
		this.features.modules = Object.keys(d.estado)
			...
			.map((k: string) => {
				...
				const estadoItem = d.estado[k] as Array<{ estado: string }>;
				this.IDEapp().push({
					title: savedK,
					logs: estadoItem.map((e) => { return { title: e.estado } })
				});

				return {
					name: k,
					englishTutorial: String(estadoItem.length),
					frenchTutorial: ''
				}
		});
	...
	sendSignal(signal: any /* SignalEvent */) {
		...
		if (signal.event == "CLIENT_SUSCRIBE") {
			this.serverService.enterRoom(signal.room);
		} else {
	...
	getMundoPaginator(m: any): {
		actual: number;
		dias: number[]
	} {

		const actual = (m?.modelo as any)?.dia || 0;
		const dias = Array.from({ length: ((m?.modelo as any)?.dia || 0) + 1}, (_, index) => index);
	...
	handleAccordionEmiter(state: GenericMap<string, unknown>) {
		// console.log("State Accordion", state)
	}

	handlePopUpEvent($event: GenericMap<string, unknown>, master: boolean) {
