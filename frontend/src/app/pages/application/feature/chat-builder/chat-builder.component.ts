...
import type { IMenuStateExtended } from '../../../../models/aaia.model';
import { ServerService } from '../../../../services/socketio/server.service';

// Completion - tipo local para chat
...
@Input()
	  get currentApp(): IMenuStateExtended {
		  return this.m_currentApp;
	  }
	  set currentApp(value: IMenuStateExtended) {
...
@Output()
	...
	constructor() {
		...
		this.serverService.IAresponses$.asObservable().subscribe(d => {
			...
			if (typeof dd.response == "string") {
				this.editor2ContentA = dd.response ?? '';
			} else {
				this.editor2ContentA = JSON.stringify(dd.response);
			}
			this.editor2ContentB = Array.isArray(dd.code) && dd.code.length > 0 ? dd.code[0] : (dd.code ?? '') + "";
