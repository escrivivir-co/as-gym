...
import type { IRuntimeBlock } from '@alephscript/mcp-core-sdk/browser';
...
export class AlephKadsComponent implements OnInit {
	...
	enterRoom(room: string) {
		this.serverService.web.io.emit("CLIENT_SUSCRIBE", { room });
	}
	...
	constructor() {
		...
		this.serverService.chainState$.subscribe((d: IRuntimeBlock) => {
			...
			const data = keys.map((k: string) => {
				...
				fs.push({
					title: k,
					logs: estadoItem.map((e) => { return { title: e.estado.substring(0, 50) } })
				});
			});
