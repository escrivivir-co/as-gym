...
import { BASIC_PROMPT_DEV_HELP, q } from "../lore";
...
import { IDEEstadoAppV1, QueryParamsJuego, setNombre } from "../appv1-estado";
...
export class LogicIdeApp extends App {
	...
	inicializar() {
		...
		this.alphaBot.automata.emitResponse = (data: any) => {
			// console.log("Launch the resonse to UI", data)
			this.spider.room("SET_IA_RESPONSE", { data }, this.getRoomName())
		 };
	...
	onAssistantsReady(as: Assistant[], caller?: string) {
		...
		this.assistantId = this.omegaBot.assistantId;

		console.log(agentMessage(this.nombre, "onAssistantsReady"), /*as, */ this.omegaBot.assistantId /*, this.receivedBots, this.expectedBots,*/,
		"=================ooooooooooooooooooooooooO=======================")
		this.spider.io.on("GET_DOMAIN_LOGIC_DATA", (...args) => {
			...
			switch(action) {
				case "SET_DATA": {
					...
					const aiMessage = rData.data.blob;

					if (aiMessage) {
						const qr: QueryParamsJuego = {
							bot_info: this.nombre,
							contexto: q.contexto,
							instrucciones: setNombre(q.instrucciones, "Bot.Omega"),
							assistant_id: this.assistantId,
							juego: {
								ram: { data: "", index: [] },
								request: /* BASIC_PROMPT_DEV_HELP + */ aiMessage,
								response: undefined,
								analytics: { report: undefined, metrics: [] }
							},
							solicitud: ""
						}
						console.log(agentMessage("ASSISTANT_IA_API", ">> Enqueue petition to agent... to: " + this.assistantId), action)
