...
export const IOT_LOGIC_ENGINE_mundo = {
	comunidad,
	"agentes": [
...
export class DAO {
	...
	realizarTransaccion(fase: string) {
	  ...
	  this.log.push({
		action: "realizarTransaccion",
		label: `DAO ${this.nombre} está en la fase: ${this.faseActual}`
	  })
	}
  }
...
export class SimuladorDAO {
	...
	getDAOsLogs(): any[] {
		let logs = [];
		this.DAOs.forEach(d => logs = logs.concat(d.log))
		return logs;
	}
	...
	actualizarEstadoMundo() {
	  ...
	  this.log.push({
		action: "actualizarEstadoMundo",
		label: `Mes: ${this.mesActual}, Estado del Mundo: ${this.estadoMundo}`
	  })
	}
	...
	iniciarJornada() {
	  ...
	  this.log.push({
		action: "iniciarJornada",
		label: `Día ${this.diaActual}: Iniciando jornada.`
	  })
	...
	finalizarJornada() {
	  ...
	  this.log.push({
		...
		label: `Día ${this.diaActual}: Finalizando jornada.`
	  })
